...
### 🌐 外部連携
- **Discord**: システム状況、メンテナンス情報をリアルタイム通知
- **LINE Bot**: スマホからAIに指示を送信。Gunicorn採用による安定稼働（NEW!）
- **ブラウザ操作**: Playwrightでウェブサイトの自動操作
- **思考共有**: 長時間の処理中にAIの現在地をLINE通知（NEW!）
...
- OLEDにLAN IPとTailscale IPを同時表示（NEW!）

### 🎙️ ハイブリッド音声システム（NEW！）
- **ナースロボ（Voicevox） × OpenAI TTS**の融合
- F15キーでシステム状態を詳細読み上げ（情報取得・生成・再生のレイテンシをログ記録）
- 独り言機能（Monologue）の搭載
