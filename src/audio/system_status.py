...
def get_system_status_text() -> str:
    ...
    try:
        ...
        # /proc/mounts を直接確認するのが最も確実
        if os.path.exists("/proc/mounts"):
            with open("/proc/mounts", "r") as f:
                if any(target_path in line for line in f):
                    is_mounted = True
                    
        if not is_mounted and os.path.ismount(target_path):
            is_mounted = True
            
        if is_mounted:
            hdd = psutil.disk_usage(target_path)
            # 0%と表示される場合、正常に取得できていない可能性があるためチェック
            if hdd.total > 0:
                if hdd.percent < 1.0 and hdd.used > 0:
                    # 1%未満だが使用されている場合
                    used_mb = hdd.used / (1024 * 1024)
                    if used_mb > 1024:
                        parts.append(f"HDDは{used_mb/1024:.1f}GB使用中。1パーセント未満で、ほぼ空っぽばい")
                    else:
                        parts.append(f"HDDは{used_mb:.0f}MB使用中。1パーセント未満で、ぜんぜん余裕ばい")
                else:
                    parts.append(f"HDDは{hdd.percent:.0f}%")
            else:
                parts.append("HDDの容量が取得できんかった…マウントが不安定かも")
        else:
            parts.append("HDDはまだマウントされとらんみたい")
    except Exception as e:
        logger.error(f"HDD status error: {e}")
        parts.append("HDDの状態が確認できんかった")
