...
# 同パッケージ/他モジュールからインポート
try:
    # 実行環境(cwd)が src などの場合
    ...
    # 実行環境が src/audio などの場合
    from stt_engine import create_stt_engine
    from tts_engine import create_tts_engine
    from recorder import Recorder
    from input_listener import InputListener, Action
    from monologue_engine import MonologueEngine
    from system_status import get_system_status_text
...
class AudioManager:
    ...
    def _init_engines(self):
        ...
        self.listener = InputListener(
            ...
            device_name=input_cfg.get("device_name"),
            key_config=key_cfg,
    ...
    def _on_action(self, action: str):
        """入力リスナーからのアクションを処理"""
        logger.info(f"[AudioManager] [AUDIO_DEBUG] Action Received: {action}")
        handlers = {
        ...
        if handler:
            ...
            logger.warning(f"[AudioManager] [AUDIO_DEBUG] Unhandled Action: {action}")
    
    def _handle_talk_press(self):
        """F13押下: 録音開始"""
        logger.info("[AudioManager] [AUDIO_DEBUG] _handle_talk_press executed.")
        if self.state == AudioState.SPEAKING:
            logger.warning("[AudioManager] [AUDIO_DEBUG] SPEAKING state prevents new recording.")
            return  # 再生中は録音禁止
    ...
    def _handle_talk_release(self):
        ...
        logger.info("[AudioManager] [AUDIO_DEBUG] _handle_talk_release executed.")
        if self.state != AudioState.LISTENING:
            logger.warning("[AudioManager] [AUDIO_DEBUG] Not LISTENING state, ignored.")
            return
        
        wav_path = self.recorder.stop()
        if not wav_path:
            logger.error("[AudioManager] [AUDIO_DEBUG] Recorder stopped but no WAV generated.")
    ...
    def _process_talk(self, wav_path: str):
        ...
        logger.info(f"[AudioManager] [AUDIO_DEBUG] Processing Talk pipeline with {wav_path}")
        try:
            # STT
            ...
            if not text:
                logger.error("[AudioManager] [AUDIO_DEBUG] STT pipeline returned empty text.")
                fail_msg = self.config.get("failsafe", {}).get(
                    "stt_fail_message", "聞き取れんかった、ごめんマスター"
                )
                self.speak(fail_msg, Priority.TALK, self.conversation_volume)
            ...
            try:
                ...
                with open(inbox_file, 'a', encoding='utf-8') as f:
                    f.write(_json.dumps(event_data, ensure_ascii=False) + "\n")
                
                logger.info(f"[AudioManager] [AUDIO_DEBUG] Inbox Send OK: {text}")
                
                self._set_state(AudioState.IDLE)
                
            except Exception as e:
                logger.error(f"[AudioManager] インボックス送信エラー: {e}")
                self.speak("うーん、ちょっと頭が回らんばい。ごめんマスター", Priority.TALK, self.conversation_volume)
                self._set_state(AudioState.IDLE)
    ...
    def _do_speak(self, req: SpeakRequest):
        """実際のTTS + 再生"""
        logger.info(f"[AudioManager] [AUDIO_DEBUG] _do_speak invoked for text: {req.text}")
        if not self.tts:
            logger.warning("[AudioManager] [AUDIO_DEBUG] TTSエンジンなし")
            return
        ...
        try:
            ...
            if stereo:
                ...
                try:
                    ...
                    logger.error(f"[AudioManager] [AUDIO_DEBUG] stereo conversion error: {e}")
            ...
            if res.returncode != 0:
                 logger.error(f"[AudioManager] [AUDIO_DEBUG] aplay failed with returncode {res.returncode}")
            else:
                 logger.info("[AudioManager] [AUDIO_DEBUG] aplay playback finished successfully.")
            ...
            logger.error(f"[AudioManager] [AUDIO_DEBUG] 再生エラー: {e}")
        finally:
            self._set_state(AudioState.IDLE)
    ...
    def _line_cmd_worker(self):
        ...
        import json as _json
        last_timestamp = ""
        logger.info(f"[LINE CMD] ワーカー開始: 監視ファイル={self.AUDIO_CMD_FILE}")
                ...
                if os.path.exists(self.AUDIO_CMD_FILE):
                    try:
                        ...
                        if ts != last_timestamp:
                            last_timestamp = ts
                            logger.info(f"[LINE CMD] コマンド受信: {cmd.get('action')}")
                        ...
                        # ファイル書き込み中の可能性があるので少し待つ
                        time.sleep(0.5)
                    except Exception as e:
                        logger.error(f"[LINE CMD] ファイル処理エラー: {e}")
            except Exception as e:
                logger.error(f"[LINE CMD] 周期処理エラー: {e}")
            
            time.sleep(0.5)  # 0.5秒間隔でポーリング
    ...
    def _handle_line_cmd(self, cmd: dict):
        ...
        if action == "speak":
            ...
            if text:
                self.speak(text, Priority.NOTIFICATION, self.conversation_volume)
                logger.info(f"[LINE CMD] [AUDIO_DEBUG] 読み上げリクエスト: {text}")
        
        elif action == "monologue_mute":
            if self.monologue:
                self.monologue.muted = True
                self.speak("独り言ミュートしたよ", Priority.NOTIFICATION, self.notification_volume)
        
        elif action == "monologue_unmute":
            if self.monologue:
                self.monologue.muted = False
                self.speak("独り言再開するね", Priority.NOTIFICATION, self.notification_volume)
