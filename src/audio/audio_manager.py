...
class AudioManager:
    ...
    def _process_talk(self, wav_path: str):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[AudioManager] インボックスへ送信完了: {text}")
                
                # インボックス送信後はIntegratedSystem側からの _send_audio_cmd("speak", ...) を待つ。
                # ここではIDLEに戻して、スピーカー入力を解放する。
                self._set_state(AudioState.IDLE)
    ...
    def _handle_line_cmd(self, cmd: dict):
        ...
        if action == "speak":
            ...
            if text:
                self.speak(text, Priority.NOTIFICATION, self.conversation_volume)
                logger.info(f"[LINE CMD] 読み上げ: {text}")
